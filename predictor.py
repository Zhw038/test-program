...
# feature_names = [
#     "TBIL.pre", "INR", "Dexmedetomidine", "Blood.loss", "RBC.t"
# ]
...
# Define feature names from the new dataset
feature_names = ["TBIL.pre", "INR", "Dexmedetomidine", "Blood.loss", "RBC.t"]
...
if st.button("Predict"):
    ...
    probability = predicted_proba[predicted_class] * 100
    advice = (
        f"According to our model, you have a {'high' if predicted_class == 1 else 'low'} risk of hyperbilirubinemia. "
        f"The model predicts that your probability of {'having' if predicted_class == 1 else 'not having'} hyperbilirubinemia is {probability:.1f}%. "
        "It's advised to consult with your healthcare provider for further evaluation and possible intervention."
    )
    st.write(advice)
